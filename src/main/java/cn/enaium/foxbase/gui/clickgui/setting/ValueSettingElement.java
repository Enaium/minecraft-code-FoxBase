...
public class ValueSettingElement extends SettingElement {
    ...
    public void render(int mouseX, int mouseY, float delta, double x, double y, double width, double height) {
        ...
        Render2D.drawRectWH(x + width + 2, y + 2, height / 2 - 4, height - 4, this.addHovered ? ColorUtils.BG : ColorUtils.SELECT);
        Render2D.drawRectWH(x + width + 2 + 8, y + 2, height / 2 - 4, height - 4, this.removeHovered ? ColorUtils.BG : ColorUtils.SELECT);
        FontUtils.drawHVCenteredString("+", x + width + 2 + width / 2, y + 2 + height / 2, Color.WHITE.getRGB());
        FontUtils.drawHVCenteredString("-", x + width + 2 + 8 + width / 2, y + 2 + height / 2, Color.WHITE.getRGB());
    }
