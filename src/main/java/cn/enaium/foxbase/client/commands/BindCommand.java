...
public class BindCommand implements ICommand {
    ...
    public boolean run(String[] args) {
        if (args.length == 3) {
            ...
            Object m = CF4M.getInstance().module.getModule(args[1]);

            if (m == null)
                return false;

            CF4M.getInstance().module.setKey(m,Keyboard.getKeyIndex(args[2].toUpperCase()));
            CF4M.getInstance().configuration.message(CF4M.getInstance().module.getName(m) + " has been bound to " + args[2] + ".");
            return true;
        }
        return false;
    }
